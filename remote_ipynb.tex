...
\begin{lstlisting}[label={lst:tunnel1}, caption={Tunnel to gateway server}]
ssh -L <PORT>:localhost:<PORT> -N <MACHINE>
\end{lstlisting}

\begin{lstlisting}[label={lst:tunnel2}, caption={Tunnel to machine}]
ssh -L <PORT>:localhost:<PORT> -N <MACHINE>
\end{lstlisting}
...
This means that the tunnel could not be established as another process is using that particular port. If this occur, start everything over again with a different port number.
